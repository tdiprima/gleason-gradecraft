...
from fastai.vision.all import (CrossEntropyLossFlat,
                               EarlyStoppingCallback, ImageDataLoaders,
                               Learner, SaveModelCallback, accuracy,
                               error_rate, resnet34, resnet50, vision_learner)
...
def load_and_split_data(
    ...
    if len(train_df) == 0:
        logger.error(
            "No valid training images found! Check your data path and filename format."
        )
        logger.error(
            f"Expected filename format: *_{{label}}.png where label is in {Config.VALID_LABELS}"
        )
...
def run_hyperparameter_search(
    ...
    study = optuna.create_study(
        direction="maximize", study_name="gleason_classification"
    )
...
def evaluate_model(
    learn: Learner, test_df: pd.DataFrame, logger: logging.Logger, output_path: Path
) -> dict:
    """
    ...
    header = "          " + " ".join(
        [f"{Config.LABEL_NAMES[i][:8]:>8}" for i in Config.VALID_LABELS]
    )
    logger.info(header)
    for i, row in enumerate(cm):
        row_str = f"{Config.LABEL_NAMES[Config.VALID_LABELS[i]][:8]:>8}: " + " ".join(
            [f"{val:>8}" for val in row]
        )
        logger.info(row_str)
    ...
    for label in Config.VALID_LABELS:
        ...
        logger.info(
            f"{name:<20} {metrics['precision']:>10.3f} {metrics['recall']:>10.3f} {metrics['f1-score']:>10.3f} {int(metrics['support']):>10}"
        )
...
def main():
    ...
    best_params = run_hyperparameter_search(train_df, valid_df, class_weights, logger)
    ...
    dls = create_dataloaders(train_df, valid_df, batch_size=best_params["batch_size"])
